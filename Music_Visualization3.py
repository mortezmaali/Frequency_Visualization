...
mp3_file_path = "C:/Users/Morteza/OneDrive/Desktop/Emily_Music/1.11.2020_Room Tones_Relaxed_Melody.mp3"
...
while pygame.mixer.music.get_busy():
    ...
    if color:  # Only update the screen if a color is returned
        # Clear the screen by filling it with black (to remove previous dots)
        screen.fill((0, 0, 0))

        # Draw random "stars" (dots) at random positions
        for _ in range(300):  # Increase the number of stars/dots for better coverage
            x = random.randint(0, screen.get_width())
            y = random.randint(0, screen.get_height())
            radius = random.randint(2, 5)  # Randomize dot size to give a "starry" effect
            
            pygame.draw.circle(screen, color, (x, y), radius)
    
        pygame.display.flip()  # Update the screen with the drawn dots
